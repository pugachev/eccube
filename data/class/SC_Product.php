...
__EOS__;
    ...
    public function setGift($productId, $giftIds)
    {
        $val['product_id'] = $productId;
        $val['creator_id'] = $_SESSION['member_id'];
        $val['create_date'] = 'CURRENT_TIMESTAMP';
        ...
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $objQuery->delete('dtb_gift', 'product_id = ?', array($productId));
        foreach ($giftIds as $giftId) {
            if ($giftId == '') continue;
            $val['gift_id'] = $giftId;
            $objQuery->insert('dtb_gift', $val);
        }
    }

    public function getGift($productIds)
    {
        if (empty($productIds)) {
            return array();
        }
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $cols = 'product_id, gift_id';
        $from = 'dtb_gift';
        $where = 'del_flg = 0 AND product_id IN (' . SC_Utils_Ex::repeatStrWithSeparator('?', count($productIds)) . ')';
        $gift = $objQuery->select($cols, $from, $where, $productIds);
        $results = array();
        foreach ($gift as $val) {
            $results[$val['product_id']][] = $val['gift_id'];
        }
