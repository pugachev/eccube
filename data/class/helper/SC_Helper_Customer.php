...
class SC_Helper_Customer
{
    ...
    public static function sfEditCustomerData($arrData, $customer_id = null)
    {
        ...
        // STEP2:会員カスタマイズ 趣味データを別の配列へ退避
        $arrHobby = $arrData['hobby_id'];
        unset($arrData['hobby_id']);
        ...
        // STEP2:会員カスタマイズ 趣味データ登録(チェックボックス)
        SC_Helper_Customer_Ex::setHobby($customer_id, $arrHobby);
    ...
    public static function sfGetCustomerData($customer_id, $mask_flg = true)
    {
        ...
        // STEP2:会員カスタマイズ 趣味データ取得(チェックボックス)
    ...
    public static function sfCustomerRegisterParam(&$objFormParam, $isAdmin = false, $is_mypage = false, $prefix = '')
    {
        ...
        // STEP2：会員カスタマイズ 項目追加
        $objFormParam->addParam('ニックネーム', $prefix . 'nick_name', STEXT_LEN, 'aKV', array('EXIST_CHECK', 'NO_SPTAB', 'SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('住まい', $prefix . 'house_id', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('ダイレクトメール', $prefix . 'dm_id', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
    ...
    public function setHobby($customerId, $hobbyIds)
    {
        ...
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $objQuery->delete('dtb_hobby', 'customer_id = ?', array($customerId));
        foreach ($hobbyIds as $hobbyId) {
            if ($hobbyId == '') continue;
            $val['hobby_id'] = $hobbyId;
            $objQuery->insert('dtb_hobby', $val);
        }
    }

    public function getHobby($customerIds)
    {
        if (empty($customerIds)) {
            return array();
        }
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $cols = 'customer_id, hobby_id';
        $from = 'dtb_hobby';
        $where = 'del_flg = 0 AND customer_id IN (' . SC_Utils_Ex::repeatStrWithSeparator('?', count($customerIds)) . ')';
        $hobby = $objQuery->select($cols, $from, $where, $customerIds);
        $results = array();
        foreach ($hobby as $val) {
            $results[$val['customer_id']][] = $val['hobby_id'];
        }
