...
class LC_Page_Admin_Products_Product extends LC_Page_Admin_Products_Ex
{
    ...
    public function lfInitFormParam(&$objFormParam, $arrPost)
    {
        ...
        // STEP1：商品カスタマイズ 項目追加
        $objFormParam->addParam('メーカーコード', 'maker_code', STEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('配送種別', 'delivery_type_id', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('保証期間', 'warranty_period_id', INT_LEN, 'n', array('NUM_CHECK'));
        $objFormParam->addParam('ギフト対応', 'gift_id', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
...
__EOF__;
        ...
        // STEP1：商品カスタマイズ ギフト対応取得
        $gift = $objProduct->getGift($product_id);
        $arrProduct[0]['gift_id'] = $gift[$product_id];
    ...
    public function lfRegistProduct(&$objUpFile, &$objDownFile, $arrList)
    {
        ...
        // STEP1：商品カスタマイズ 項目追加
        $sqlval['maker_code'] = $arrList['maker_code'];
        $sqlval['delivery_type_id'] = $arrList['delivery_type_id'];
        ...
        // STEP1：商品カスタマイズ ギフト設定
        $objProduct = new SC_Product_Ex();
        $objProduct->setGift($product_id, $arrList['gift_id']);
