...
class LC_Page_Admin_Products extends LC_Page_Admin_Ex
{
    ...
    public function init()
    {
        ...
        // STEP1：商品カスタマイズ 項目追加
        $this->arrDeliveryType = $masterData->getMasterData('mtb_delivery_type');
        $this->arrWarrantyPeriod = $masterData->getMasterData('mtb_warranty_period');
    ...
    public function lfInitParam(&$objFormParam)
    {
        ...
        // STEP1：商品カスタマイズ 項目追加
        $objFormParam->addParam('メーカーコード', 'search_maker_code', STEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('配送種別', 'search_delivery_type_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('保証期間', 'search_warranty_period_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK'));
        $objFormParam->addParam('ギフト対応', 'search_gift_id', INT_LEN, 'n', array('MAX_LENGTH_CHECK'));
    ...
    public function buildQuery($key, &$where, &$arrValues, &$objFormParam, &$objDb)
    {
        ...
        switch ($key) {
            ...
            case 'search_product_statuses':
                ...
                break;
            // STEP1：商品カスタマイズ メーカーコード
            case 'search_maker_code':
                $where .= ' AND maker_code LIKE ?';
                $arrValues[] = sprintf('%%%s%%', $objFormParam->getValue($key));
                break;
            // STEP1：商品カスタマイズ 配送種別
            case 'search_delivery_type_id':
                $where.= ' AND delivery_type_id = ?';
                $arrValues[] = $objFormParam->getValue($key);
                break;
            // STEP1：商品カスタマイズ 保証期間
            case 'search_warranty_period_id':
                $tmp_where = '';
                foreach ($objFormParam->getValue($key) as $element) {
                    if ($element != '') {
                        if (SC_Utils_Ex::isBlank($tmp_where)) {
                            //最初はここから始まる
                            $tmp_where .= ' AND (warranty_period_id = ?';
                        } else {
                            //次以降はここを通る
                            $tmp_where .= ' OR warranty_period_id = ?';
                        }
                        //バインド(?の箇所)する検索値
                        $arrValues[] = $element;
                    }
