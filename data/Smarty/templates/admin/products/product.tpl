...
<div id="products" class="contents-main">
    ...
    <table class="form">
        ...
        <tr>
            <th>メーカーコード</th>
            <td>
                <span class="attention"><!--{$arrErr.maker_code}--></span>
                <input type="text" name="maker_code" value="<!--{$arrForm.maker_code|h}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" style="<!--{if $arrErr.maker_code != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}-->;<!--{/if}-->" size="60" class="box60" />
                <span class="attention"> (上限<!--{$smarty.const.STEXT_LEN}-->文字)</span>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <span class="attention"><!--{$arrErr.delivery_type_id}--></span>
                <select name="delivery_type_id" style="<!--{$arrErr.delivery_type_id|sfGetErrorColor}-->">
                    <option value="">選択してください</option>
                    <!--{html_options options=$arrDeliveryType selected=$arrForm.delivery_type_id}-->
                </select>
            </td>
        </tr>
        <tr>
            <th>保証期間</th>
            <td>
            <span class="attention"><!--{$arrErr.warranty_period_id}--></span>
            <!--{html_radios name="warranty_period_id" options=$arrWarrantyPeriod selected=$arrForm.warranty_period_id separator='&nbsp;&nbsp;'}-->
            </td>
        </tr>
        <tr>
            <th>ギフト対応</th>
            <td>
            <span class="attention"><!--{$arrErr.gift_id}--></span>
            <!--{html_checkboxes name="gift_id" options=$arrGift selected=$arrForm.gift_id separator='&nbsp;&nbsp;'}-->
            </td>
        </tr>
    </table>
