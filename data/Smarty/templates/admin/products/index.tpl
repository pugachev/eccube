    ...
    <form name="search_form" id="search_form" method="post" action="?">
        ...
        <table>
            ...
            <tr>
                <th>メーカーコード</th>
                <td>
                    <!--{assign var=key value="search_maker_code"}-->
                    <!--{if $arrErr[$key]}-->
                        <span class="attention"><!--{$arrErr[$key]}--></span>
                    <!--{/if}-->
                    <input type="text" name="<!--{$key}-->" value="<!--{$arrForm[$key].value|h}-->" maxlength="<!--{$arrForm[$key].length}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->" size="30" class="box30" />
                ...
                <th>配送種別</th>
                <td>
                    <!--{assign var=key value="search_delivery_type_id"}-->
                    <span class="attention"><!--{$arrErr[$key]}--></span>
                    <select name="<!--{$key}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->">
                    <option value="">選択してください</option>
                    <!--{html_options options=$arrDeliveryType selected=$arrForm[$key].value}-->
                    </select>
                </td>
                <tr>
                    <th>保証期間</th>
                    <td>
                        <!--{assign var=key value="search_warranty_period_id"}-->
                        <span class="attention"><!--{$arrErr[$key]|h}--></span>
                        <!--{html_checkboxes name="$key" options=$arrWarrantyPeriod selected=$arrForm[$key].value}-->
                    </td>
                    <th>ギフト対応</th>
                    <td>
                        <!--{assign var=key value="search_gift_id"}-->
                        <span class="attention"><!--{$arrErr[$key]|h}--></span>
                        <!--{html_checkboxes name="$key" options=$arrGift selected=$arrForm[$key].value}-->
                    </td>
                </tr>
            </tr>
        </table>
